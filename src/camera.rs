...
impl CameraState {
	...
	fn update_pos(&mut self, t: f32) {
		macro_rules! displace {
		    ($x:expr, $y:expr, $z:expr) => (
				if self.keys.contains($z) {
					let displacement = vec3_scale($x, $y);
		...
		if self.keys.contains(&self.settings.move_faster_button) {
			dist *= self.fast_scale;
		}

		displace!(self.forward, dist, &self.settings.move_backward_button);
		displace!(self.forward, -dist, &self.settings.move_forward_button);

		displace!(self.right, dist, &self.settings.strafe_left_button);
		displace!(self.right, -dist, &self.settings.strafe_right_button);

		displace!(self.up, dist, &self.settings.fly_up_button);
		displace!(self.up, -dist, &self.settings.fly_down_button);	
	}
